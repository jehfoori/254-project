...
uint32_t expected_row(uint32_t rank_x, uint32_t rank_y) {
  uint32_t rank = rank_y * kTeamX + rank_x;
  return (rank_x == 0) ? (0x100 + rank) : (0x100 + rank - 1);
}

uint32_t expected_col(uint32_t rank_x, uint32_t rank_y) {
  uint32_t rank = rank_y * kTeamX + rank_x;
  return (rank_y == 0) ? (0x200 + rank) : (0x200 + rank - kTeamX);
}
  ...
  for (uint32_t block_y = 0; block_y < kGridY; ++block_y) {
    for (uint32_t block_x = 0; block_x < kGridX; ++block_x) {
      ...
      if (result.team_id != 0
       ...
       || result.row_value != expected_row(block_x, block_y)
       || result.col_value != expected_col(block_x, block_y)
       || result.status != 1) {
        std::cout << "*** error: block (" << block_x << ", " << block_y << ")"
                  << " team_id=" << result.team_id
                  << " rank=(" << result.team_rank_x << ", " << result.team_rank_y << ")"
                  << " row=0x" << std::hex << result.row_value
                  << " col=0x" << result.col_value
                  << " status=" << std::dec << result.status
                  << std::endl;
        ++errors;
      }
    }
